...
class ClassifierAnalyzer(
    ...
    override fun analyze(imageProxy: ImageProxy) {
        try {
            ...
            // Pindahkan logic inferensi ke fungsi terpisah,
            // supaya bisa dipakai juga oleh MultiModelAnalyzer
            analyzeBitmap(bitmap)
        } catch (e: Exception) {
            Log.e("ClassifierAnalyzer", "err: ${e.message}", e)
        } finally {
            // Penting: tetap close di sini kalau dipakai langsung sebagai Analyzer CameraX
            imageProxy.close()
        }
    }

    /**
     * Fungsi ini yang nanti akan dipanggil MultiModelAnalyzer.
     * DI SINI tidak ada imageProxy.close(), karena kita cuma pakai Bitmap.
     */
