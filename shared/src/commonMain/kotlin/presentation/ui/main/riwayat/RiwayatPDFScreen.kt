...
private fun normalizePdfUrl(input: String): String {
    return input
        .replace("\\/", "/")              // hapus escaped slash
        .replace(Regex("^https:\\/\\/https:\\/\\/"), "https://") // buang double https jika ada
        .trim()
}
...
private fun RiwayatPDFContent(
    ...
    if (state.isSendEmailDialogOpen) {
        SendEmailDialog(
            ...
            onDismissRequest = {
                events(RiwayatEvent.HideSendEmailDialog)
            },
            onSendClick = { emails, sendToMyEmail ->
                events(RiwayatEvent.HideSendEmailDialog)
                events(RiwayatEvent.SendEmailBA(emails, sendToMyEmail))
            }
        )
    }
...
//                .padding(16.dp)
//        ) {
//            // 2. Gunakan URL yang sudah diproses oleh REGEX
//            Base64PdfViewer(
//                url = normalizePdfUrl(pdfUrl),
//                modifier = Modifier.fillMaxSize()
//            )
...
//                OutlinedButton(
//                    onClick = {
////                        events(RiwayatEvent.SendEmailBA)
//                        events(RiwayatEvent.ShowSendEmailDialog)
//                    },
//                    modifier = Modifier
//                        .weight(1f)
