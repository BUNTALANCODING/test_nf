...
fun ListInputCore(
    ...
    var inputValues by remember { mutableStateOf(List(inputLength) { "" }) }
    val focusRequesters = List(inputLength) { FocusRequester() }

    LaunchedEffect(clearOtpTrigger) {
        if (clearOtpTrigger) {
            inputValues = List(inputLength) { "" }
        }
    }
            ...
            TextField(
                ...
                onValueChange = { newValue ->
                    if (newValue.length <= 1 && !newValue.any { it.isLetter() }) {
                        val updatedOtp = inputValues.toMutableList()
                        updatedOtp[index] = newValue
                        inputValues = updatedOtp

                        if (newValue.isNotEmpty() && index < inputLength - 1) {
                            focusRequesters[index + 1].requestFocus()
                        }

                        if (updatedOtp.joinToString("").length == inputLength) {
