...
class RampcheckStartUseCase(
    private val service: MainService,
    private val appDataStoreManager: AppDataStore,
) : BaseUseCase<RampcheckStartRequestDTO, RampcheckStartDTO, RampcheckStartDTO>(appDataStoreManager) {
    override suspend fun run(
        params: RampcheckStartRequestDTO,
        token: String
    ): MainGenericResponse<RampcheckStartDTO> = service.rampcheckStart(
        token = token,
        request = params
    )
